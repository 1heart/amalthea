...
%   Adrian M. Peter
%--------------------------------------------------------------------------
function slidingDist = wde2DSlidingDist(coeffs1, coeffs2, wdeSet,lambda)
outputFileNameAssign = 'out.txt';
inputFileNameAssign = 'in.txt';

wName = wdeSet.wName;
startLevel = wdeSet.startLevel;
stopLevel = wdeSet.stopLevel;
coeffsIdx = wdeSet.coeffsIdx;
% coeffsIdx = get_numtranslates_per_dataset(wdeSet);
% coeffsIdx = coeffsIdx{1};
% coeffsIdx
sampleDataSupp = wdeSet.sampleSupp;

% execPath = '.';
% inputFileNameCostMat = 'input.txt';
% inputFileNameCostMat = 'output.txt';
...
% decompositions.
[distMats basesLocs] = euclidDistsBetwCoeffs(wdeSet);

function [dFx,dFy,dFz,W]= analyticSqrtDerivOfMultiGaussian(domain,r,discretization,sample)
...
F1 = -.5*W(:,1).*sqrt(exp(-.5*(W(:,1).^2 + W(:,2).^2 + W(:,3).^2)));
F2 = -.5*W(:,2).*sqrt(exp(-.5*(W(:,1).^2 + W(:,2).^2 + W(:,3).^2)));
F3 = -.5*W(:,3).*sqrt(exp(-.5*(W(:,1).^2 + W(:,2).^2 + W(:,3).^2)));

dFx = sqrt(Constant)*F1;
dFy = sqrt(Constant)*F2;
dFz = sqrt(Constant)*F3;

  figure (21)% here r small will make it good de
    % % reshape density
...
%     maxOfTrueDensity = max(dFx(:));
    ...
    ylabel('y coordinate')
    zlabel('z cordinate')
    title ([num2str(dim),'D plot of the true derivative of the sqrt density w.r.t X'])
    grid on
    xlabel('x coordinate')  
figure (23)
...
%     maxOfTrueDensity = max(dFx(:));
    ...
    ylabel('y coordinate')
    zlabel('z cordinate')
    title ([num2str(dim),'D plot of the true derivative of sqrt density w.r.t Y'])
    grid on
    xlabel('x coordinate')  
...
%     maxOfTrueDensity = max(dFx(:));
    ...
    ylabel('y coordinate')
    zlabel('z cordinate')
    title ([num2str(dim),'D plot of the true derivative of the sqrt density w.r.t Z'])
    grid on
    xlabel('x coordinate')  
